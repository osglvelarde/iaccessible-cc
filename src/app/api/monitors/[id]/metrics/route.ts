...
import { MonitorMetricsResponse } from '@/lib/types/monitoring';
...
export async function GET(
  ...
  try {
    ...
    // Get range from query params (default to 24h)
    ...
    const result = await kumaClient.getMonitorMetrics(monitorId, range);
    ...
    const response: MonitorMetricsResponse = {
      metrics: result.data!,
      range
    };
